# slope-calculator — CLAUDE.md
**Version:** 1.3.0
**Initialized:** 2026-04-19T06:24:35-0400
...
| #   | Rule                                                                 | Rationale                                                     |
|-----|----------------------------------------------------------------------|---------------------------------------------------------------|
| C1  | Never push to main directly; always PR even for trivial edits       | Pre-launch, main is the only clean branch                    |
| C2  | Dev server port is 17449, never 3000/5173/8080                      | Legacy AI port-allocation policy                             |
| C3  | Bundle must stay ≤ 150 KB gzipped (JS + CSS combined)              | Field-performance requirement for slow mobile connections     |
| C4  | Never add dependencies without AGPL-3.0-or-later compatibility check | License invariant from plan                                  |
| C5  | All 11 calc engines are pure functions — never add side effects     | Testability + Trade Commons API shares the same engines      |
| C6  | FLUM compute() is the One Door In — never bypass it                 | Architectural invariant; all tool routing must go through it  |
| C7  | Entity extraction must remain regex-only (no LLM runtime dep)      | Works offline; no API dependency for text parsing            |
| C8  | No accounts, no telemetry, no tracking pixels, no email capture    | Non-negotiable invariant from plan                           |
| C9  | Spanish strings must be trade-fluent, not machine-translated        | Workforce is substantially non-native-English-speaking       |
